...
A website to display the results of running rsmetacheck-bot on different organizations
...
python -m pip install "pystache>=0.6.8" "rsmetacheck-bot>=0.6.0"
```
...
In fallback mode, it installs required Python dependencies (`rsmetacheck`, `rsmetacheck-bot`, `pystache`) with `pip` before running.
